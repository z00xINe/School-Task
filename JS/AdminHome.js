...
var TeacherValue = 1;
window.localStorage.value = TeacherValue;

document.addEventListener("DOMContentLoaded", function () {
    let storedUsers = JSON.parse(window.localStorage.user) ;
    for(let i = 0; i <storedUsers.length; i++ ){
        if (storedUsers[i].type == "Teacher") {
            var optgroup = document.getElementById("optgroup");
            var newOption = document.createElement("option");
            newOption.textContent = storedUsers[i].username;
            newOption.value = window.localStorage.value;
